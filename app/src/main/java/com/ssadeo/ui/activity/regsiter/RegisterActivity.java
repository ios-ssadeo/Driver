...
import com.ssadeo.ui.activity.welcome.WelcomeActivity;
...
import static com.ssadeo.common.Constants.APP_REQUEST_CODE;
...
public class RegisterActivity extends BaseActivity implements RegisterIView {
    ...
    //Arun changes
    @BindView(R.id.rtGhanian)
    RadioButton rtGhanian;
    @BindView(R.id.rtNGhanian)
    RadioButton rtNGhanian;
    @BindView(R.id.rtMale)
    RadioButton rtMale;
    @BindView(R.id.rtFemale)
    RadioButton rtFemale;
    @Override
...
//        }
    ...
    void register() {
        ...
        gender = rtMale.isChecked()? "MALE":"FEMALE";
        nationality = rtGhanian.isChecked()? "1":"0";
        ...
        //Natioanlity and Gender parameters
        map.put("gender", toRequestBody(gender));
        map.put("ghanaian", toRequestBody(nationality));
        ...
        switch (view.getId()) {
                ...
                } else */
            {
                if (Validation(txtPhoneNumber.getText().toString().trim())) {
                    ...
                    Toasty.error(this, getString(R.string.invalid_last_name), Toast.LENGTH_SHORT, true).show();
                    return;
                }else  if (!rtGhanian.isChecked() && !rtNGhanian.isChecked()) {
                    Toasty.error(this, getString(R.string.select_nationality), Toast.LENGTH_SHORT, true).show();
                    return;
                }
                else
                    if (!rtMale.isChecked() && !rtFemale.isChecked()) {
                    Toasty.error(this, getString(R.string.select_gender), Toast.LENGTH_SHORT, true).show();
                    return;
                }

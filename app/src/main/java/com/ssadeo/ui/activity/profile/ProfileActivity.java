...
import com.ssadeo.ui.activity.change_password.ChangePasswordActivtiy;
...
import static com.ssadeo.common.Constants.MULTIPLE_PERMISSION;
import static com.ssadeo.common.Constants.RC_MULTIPLE_PERMISSION_CODE;
...
public class ProfileActivity extends BaseActivity implements ProfileIView, EasyPermissions.PermissionCallbacks {
    ...
    void profileUpdate() throws IOException {
        ...
        try {
            FileUtils.copyFile(compressedImageFile, desFile);
        } catch (IOException e) {
            e.printStackTrace();
        }
        MultipartBody.Part filePart = null;
        if (imgFile != null)
            filePart = MultipartBody.Part.createFormData("avatar", desFile.getName(), RequestBody.create(MediaType.parse("image*//*"), desFile));

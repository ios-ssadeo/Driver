...
import com.ssadeo.common.chat.ChatActivity;
import com.ssadeo.data.network.model.Request_;
import com.ssadeo.data.network.model.UserGetResponse;
import com.ssadeo.ui.activity.main.MainActivity;
import com.ssadeo.ui.bottomsheetdialog.cancel.CancelDialogFragment;
    ...
    void statusUpdateCall(String status) {
        if (DATUM != null) {
            ...
            if (status.equalsIgnoreCase("DROPPED")) {
                map.put("latitude", SharedHelper.getKey(requireActivity(), "current_latitude"));
                map.put("longitude", SharedHelper.getKey(requireActivity(), "current_longitude"));
                map.put("address", getAddress(new LatLng(Double.parseDouble(SharedHelper.getKey(requireActivity(), "current_latitude")), Double.parseDouble(SharedHelper.getKey(activity(), "current_longitude")))));
            }
